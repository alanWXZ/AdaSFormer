...
class DecoderBN(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            bottleneck_features, features, kernel_size=1, stride=1, padding=0
        )

        self.out_feature_1_1 = out_feature  # 200
        self.out_feature_1_2 = out_feature  # 200
        self.out_feature_1_4 = out_feature  # 200
        self.out_feature_1_8 = out_feature  # 200
        self.out_feature_1_16 = out_feature  # 200
    ...
    def forward(self, features):
        x_block0, x_block1, x_block2, x_block3, x_block4 = (
            ...
            features[6],
            features[8],
            features[11],  # (B, 2560, 15, 20)
        )
        bs = x_block0.shape[0]
        x_d0 = self.conv2(x_block4)  # (B, 2560, 15, 20)

        if self.use_decoder:
            x_1_16 = self.up16(x_d0, x_block3)  # (B, 1280, 30, 40)

            x_1_8 = self.up8(x_1_16, x_block2)  # (B, 640, 60, 80)
...
class Encoder(nn.Module):
    def __init__(self, backend, frozen_encoder=False):
...
class UNet2D(nn.Module):
    def __init__(self, backend, num_features, out_feature, use_decoder=True, frozen_encoder=False):
        super(UNet2D, self).__init__()
        self.use_decoder = use_decoder
        
        self.encoder = Encoder(backend, frozen_encoder)
    ...
    def build(cls, **kwargs):
        ...
        # )
        basemodel = torch.hub.load(
            '/home/psdz/.cache/torch/hub/rwightman_gen-efficientnet-pytorch_master/', basemodel_name, source='local',
            pretrained=True
        )
        print("Done.")
...
if __name__ == '__main__':
    ...
    tensor = torch.ones((4,3,360,480))
    feature2d = model(tensor)
