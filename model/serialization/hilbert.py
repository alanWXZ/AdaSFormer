...
def encode(locs, num_dims, num_bits):
    ...
    #locs_uint8 = locs.long() >> (torch.arange(8, dtype=torch.int64) * 8) & 0xFF

    locs_uint8 = ((locs.long().unsqueeze(-1) >> (torch.arange(8, dtype=torch.int64) * 8).to(locs.device))
                  & 0xFF).to(torch.uint8)
    ...
    hh_uint64 = hh_uint8.to(torch.int64)
    h_shift =  (torch.arange(8, dtype=torch.int64, device=hh_uint64.device) * 8)
    hh_uint64 = (hh_uint64 << h_shift).sum(dim=-1)
